  ...
  return (
    ...
    >
      ...
      <p className="mt-3 leading-relaxed text-ink-500">
        Stop rewriting the same details into a different form every time. Set your profile up once,
        then apply with your thumb.
      </p>

      <ul className="mt-6 space-y-3">
        <Benefit>Apply in one swipe — resume attached automatically</Benefit>
        <Benefit>Only see roles that match your salary and location floor</Benefit>
...
function EmployerPanel() {
  return (
    ...
    >
      ...
      <h2 className="mt-5 text-3xl font-extrabold tracking-tight">I&apos;m hiring</h2>
      <p className="mt-3 leading-relaxed text-white/70">
        Post a role and it lands in matching candidates&apos; decks immediately. Applicants arrive
        with a resume already attached — nothing to chase.
      </p>
      ...
      <ul className="mt-6 space-y-3">
        ...
        <Benefit dark>Resumes are private — only you see your applicants</Benefit>
      </ul>
...
function Benefit({ children, dark = false }: { children: React.ReactNode; dark?: boolean }) {
  return (
    <li className="flex gap-3">
      ...
      <span className={`leading-relaxed ${dark ? 'text-white/80' : 'text-ink-700'}`}>
        {children}
      </span>
    </li>
  );
