...
/**
 * Next page of swipeable cards.
 *
 * `exclude` carries the ids already buffered on the client, so a background
 * top-up never returns a card that is still on screen.
 */
 ...
 * half-applied state isn't reachable. Idempotent per (job, seeker).
 */
export async function applyToJob(
  ...
  if (error) throw error;
  if (!data) throw new Error("The server did not return the application.");
  return data;
}
...
/**
 * Employer dashboard list. Applicant counts come back as an embedded aggregate
 * so the screen doesn't need one query per posting.
 */
